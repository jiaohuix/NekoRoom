...
void LAppLive2DManager::OnUpdate(LAppModelParameters parameters) const
{
    ...
    for (csmUint32 i = 0; i < modelCount; ++i)
    {
        ...
        // v25 诊断：throttled log（每 60 帧 ~1 秒）—— 单行紧凑格式
        // 输出：Java 输入 + projection 矩阵前 8 个关键值（m00/m03/m11/m13/m22/m23/m33）
        // 行格式：DIAG type=prj ... | DIAG type=mxin ...
        // 注意：MultiplyByMatrix 在这之后，所以这里 log 的是 raw projection
        static int s_frameCounter = 0;
        if (++s_frameCounter % 60 == 0)
        {
            const csmFloat32* p = projection.GetArray();
            __android_log_print(ANDROID_LOG_INFO, "MoeAvatar.Live2D",
                "DIAG type=mxin scale=%.3f tx=%.3f ty=%.3f winW=%d winH=%d whRatio=%.3f",
                parameters.modelScale, parameters.modelTranslateX, parameters.modelTranslateY,
                width, height, static_cast<float>(width) / static_cast<float>(height));
            __android_log_print(ANDROID_LOG_INFO, "MoeAvatar.Live2D",
                "DIAG type=prj m00=%.3f m03=%.3f m11=%.3f m13=%.3f m22=%.3f m23=%.3f m33=%.3f",
                p[0], p[3], p[5], p[7], p[10], p[11], p[15]);
        }

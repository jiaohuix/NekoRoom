...
void LAppModel::Update(LAppModelParameters parameters)
{
    ...
    if (parameters.clearOutfit) {
        ClearOutfit();
    }
    else if (parameters.changeOutfit) {
        if (_debugMode)
        {
...
void LAppModel::Draw(CubismMatrix44& matrix)
{
    ...
    // v25 诊断：throttled log（每 60 帧 ~1 秒）—— 打印 modelMatrix 关键值（投到最终 mvp 之前）
    static int s_drawFrameCounter = 0;
    if (++s_drawFrameCounter % 60 == 0)
    {
        const csmFloat32* mm = _modelMatrix->GetArray();
        __android_log_print(ANDROID_LOG_INFO, "MoeAvatar.Live2D",
            "DIAG type=mdl m00=%.3f m03=%.3f m11=%.3f m13=%.3f m22=%.3f m23=%.3f m33=%.3f",
            mm[0], mm[3], mm[5], mm[7], mm[10], mm[11], mm[15]);
    }
    ...
    if (s_drawFrameCounter % 60 == 0)
    {
        const csmFloat32* m = matrix.GetArray();
        __android_log_print(ANDROID_LOG_INFO, "MoeAvatar.Live2D",
            "DIAG type=mvp m00=%.3f m03=%.3f m11=%.3f m13=%.3f m22=%.3f m23=%.3f m33=%.3f",
            m[0], m[3], m[5], m[7], m[10], m[11], m[15]);
    }
...
void LAppModel::ClearOutfit()
{
    if (_debugMode)
    {
        LAppPal::PrintLog("[APP]clear outfit -> model default.");
    }
    if (_outfitExpressionManager != NULL)
    {
        _outfitExpressionManager->StopAllMotions();
    }
}
...
void LAppModel::SetupTextures()
{
    for (csmInt32 modelTextureNumber = 0; modelTextureNumber < _modelSetting->GetTextureCount(); modelTextureNumber++)
    {
        ...
        if (texture == nullptr)
        {
            __android_log_print(ANDROID_LOG_ERROR, "MoeAvatar.Live2D", "skip missing model texture: %s", texturePath.GetRawString());
            continue;
        }

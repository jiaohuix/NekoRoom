...
LAppTextureManager::TextureInfo* LAppTextureManager::CreateTextureFromPngFile(std::string fileName)
{
    ...
    {
        __android_log_print(ANDROID_LOG_ERROR, "MoeAvatar.Live2D", "texture missing: %s", fileName.c_str());
        return nullptr;
    }
    ...
    if (png == nullptr)
    {
        __android_log_print(ANDROID_LOG_ERROR, "MoeAvatar.Live2D", "texture decode failed: %s", fileName.c_str());
